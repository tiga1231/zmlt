...
##lastfm
fns = natsorted(glob("./data/txt/lastfm/*.txt"))[:8]
##topics
...
def radial_layout(
    g, root=None, mode="center", origin=[0, 0], phase0=0, range0=np.pi * 2
):
    g0 = g
    ...
    while len(pos) < len(g.nodes):
        ...
        for root in roots:
            if mode == "ordered":
                neighbors = [
                    n for n in g0.nodes[root]["neighbor_order"] if n not in pos
                ]
            else:
            ...
            if len(neighbors) > 0:
                ...
                newPos, newPhases, newRanges = fan(
                    ...
                    origin=origin,
                    radii=[depth for e in edge_lengths],  # mode: mw
...
# rotate layout manually for figure
rot = -100
...
# Compute scale of initial layout according to desired_edge_lengths
node_order = range(len(g.nodes))
desired_edge_lengths = {(e0, e1): data["weight"] for e0, e1, data in g.edges(data=True)}
actual_edge_lengths = {
    (e0, e1): dist(pos0[e0], pos0[e1]) for e0, e1, data in g.edges(data=True)
}
actual_desire_ratio = {
    (e0, e1): actual_edge_lengths[e0, e1] / desired_edge_lengths[e0, e1]
    for e0, e1 in desired_edge_lengths
}
min_ratio = np.min(list(actual_desire_ratio.values()))
scale = 1 / min_ratio
pos = {k: list(np.array(v) * scale) for k, v in pos0.items()}


## to JSON
dir_out, name = str(Path(fn).parent), Path(fn).stem
dir_out = dir_out.replace("dot", "json").replace("txt", "json")
suffix = increment_mode
dir_out += f"_{suffix}"
fn_out = dir_out + "/" + name
# fn_out += f"-{int(time())}" ## add a timestamp to output file
