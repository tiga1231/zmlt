...
/// External, precomputed layouts for figures
/// DELG
// let fns = [
...
function init(data) {
  ...
  // auto start, progressive mode
  // canvas.worker.postMessage({
  //   type: 'auto-add-nodes'
  // });
  //auto start, non-progressive mode
  canvas.worker.postMessage({
    type: "restart",
  });
...
function markNonOverlapLevels(canvas) {
  ...
  let l0 = 0;
  // for (let l of [10, 20]){
  let levels = Array.from(new Set(nodes.map((d) => d.level))).sort((a, b) =>
    a - b
  );
  for (let l of levels) {
    let nodes_l = nodes
...
function initSimulationWorker(canvas, simData) {
  ...
  worker.onmessage = function (event) {
    ...
    if (type === "tick") {
      ...
      console.log(
        `${(data.progress * 100).toFixed(2)}%, runtime: ${
          (runtime / 1000).toFixed(2)
        } sec`,
      );
...
function debugMsg() {
  let edgesTmp = window.edges.filter((e) =>
    window.enabledNodes.has(e.source.id) && window.enabledNodes.has(e.target.id)
  );
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (key === "p") { //[P]ause
      ...
      if (shouldMarkOverlap) {
        markLabelByOverlap(
          canvas.data.nodes.filter((d) => d.shouldShowLabel),
          canvas,
        );
      }
      canvas.draw(shouldLabel, forceLabel, shouldMarkOverlap);
      ...
      let iel = bestIdealEdgeLengthPreservation(
        edges,
...
function evalMsg(nodes, edges, labelTexts, counts = undefined) {
  ...
  for (let nc of counts) {
    ...
    let edgesTmp = edges.filter((e) =>
      e.source.nodeCount <= nc && e.target.nodeCount <= nc
    );
    let nodesTmp = nodes.filter((d) => d.nodeCount <= nc);
    let bboxesTmp = labelTexts.filter((d) => d.nodeCount <= nc).nodes().map(
      (d) => d.getBoundingClientRect()
    );
    let idealEdgeLength = bestIdealEdgeLengthPreservation(
      edgesTmp,
      edgesTmp.map((e) => e.weight),
    );
    ...
    console.log(
      ...
      "\n",
      "edge length:",
      parseFloat(idealEdgeLength.toFixed(4)),
      "\n",
      "area utilization:",
  ...
  for (let i = 0; i < dl.length; i++) {
    table += `\\textbf{$T_${i + 1}$} & ${dl[i].toFixed(2)} & ${
      cm[i].toFixed(4)
    }\\\\ \\hline \n`;
    table += `${i + 1} & ${dl[i].toFixed(2)} & ${
      cm[i].toFixed(4)
    } \\\\hline]\n`;
  }
  console.log(table);
...
function markLabelByOverlap(nodes, canvas) {
  ...
  let shown = new Set(
    nodes.filter((d) => d.shouldShowLabel).map((d) => d.index),
  );
  for (let ni of nodes) {
...
function draw(label = true, forceLabel = false, markOverlap = true) {
  ...
  if (this.data.nodesByLevel === undefined) {
    this.data.nodesByLevel = data.nodes.slice().sort((a, b) =>
      a.level - b.level
    );
  }
...
function drawEdges(ctx, edges, scales, transform) {
  ...
  for (let e of edges) {
    ...
    let lw = Math.max(
      0.5,
      scales.sr(Math.min(e.source.level, e.target.level)) / 2,
    );
    ctx.lineWidth = lw * k * DPR;
    ctx.globalAlpha = e.source.update && e.target.update
      ? 1.0
      : HIDDEN_EDGE_ALPHA;
    ctx.beginPath();
    ctx.moveTo(x0, y0);
